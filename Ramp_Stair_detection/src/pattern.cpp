...
//		CvMat rotationMatrix = rotMat; // projectionMatrix = [rotMat tra];
	...
	void Pattern::setPoints(const Mat& camMatrix, const Mat& distMatrix,Point vertexpoints[4])
	{
		...
		Point vertexPoints[4];
		for (int i =0; i<4; i++){
			vertexpoints[i] = model2ImagePts.at(i%4);
		}
	}
	void Pattern::draw(Mat& frame, const Mat& camMatrix, const Mat& distMatrix, Point points[4])
	{
		...
		*/
		...
		for (i =0; i<4; i++){
			...
			points[i] = model2ImagePts.at(i%4);
		}
		for (i =4; i<7; i++){
